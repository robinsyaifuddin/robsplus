...
const NavItem = ({ href, text, isActive, onClick, isDropdown, dropdownItems }: NavItemProps) => {
  ...
  if (isDropdown) {
    return (
      ...
      >
        <div 
          className={`flex items-center text-sm font-medium transition-colors hover:text-cyber-lightBlue cursor-pointer ${
        ...
        {isHovering && (
          <div className="absolute top-full left-0 mt-1 w-48 rounded-md shadow-lg overflow-hidden z-50">
            <div className="bg-cyber-darkBlue border border-cyber-lightBlue/20 rounded-md py-1">
              {dropdownItems?.map((item) => (
                <Link
                  key={item.text}
                  to={item.href}
                  className="block px-4 py-2 text-sm text-white hover:bg-cyber-blue/10 hover:text-cyber-lightBlue"
                >
                  {item.text}
                </Link>
              ))}
